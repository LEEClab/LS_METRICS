    ...
    for r in res:
        ...
        for i in methods:
            ...
            if method.count(i) == 0:
                if i == 'renyi':
                    for alp in alpha:
                        print('map_in:',map_in,' map_out: ',map_out ,' alp:',alp,' overw',overw)
                        grass.run_command('r.li.renyi', input=map_in,
                                          output=map_out + '_renyi_size_' + r \
                                          + '_alpha_' + str(alp), alpha=alp,
                                          conf='conf_diversity_' + r,
...
def createConfFile(res, inpumap, home):
    ...
    os.chdir(r'C:\_____dados\Space__Gits\LS_METRICS\DB_demo\resultTemp')
    fileConf = open(confilename, 'w')
    # write file
    fileConf.write('teste')
    # close file
    fileConf.close()
    ...
    #outputLine.append('SAMPLEAREA -1|-1| {} | {} \n'.format(str(rV), str(rV) ))
