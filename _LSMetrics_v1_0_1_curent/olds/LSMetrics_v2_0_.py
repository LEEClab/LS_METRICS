...
"""
 ...
 Copyright (C) 2015-2017 by Milton C. Ribeiro, John W. Ribeiro, Bernardo B. S. Niebuhr, and Mauricio H. Vancine.
...
import subprocess
# classes de divesity
from class_Shannon import ShannonFunction
from class_Pielou import PielouFunction
from class_Simpson import SimpsonFunction
from class_Renyi import RenyiIndex
...
def createtxt(input_map, outputfolder, filename = ''):
  ...
  if len(y) != 0:
    ...
    for i in range(len(idd)):
      ...
      txt_file.write('{},{},{}\n'.format(idd[i],areas[i],percentages[i]))
...
def fragment_area(input_maps, list_edge_depths,
  ...
  for i_in in input_maps:
    ...
    for a in list_corridor_width_pixels:
      ...
      format_escale_name = '0000{}'.format(meters)
      ...
      if remove_trash:
        ...
        if zero:
          ...
          txts = ["{}_ero_{}{}".format(i,format_escale_name,'m'),"{}_dila_{}{}".format(i,format_escale_name,'m'), "{}_FRAG_{}{}".format(i,format_escale_name,"m_pos"),"{}_FRAG_{}{}".format(i,format_escale_name,"m_pos_habitat"), '{}_{}{}'.format(i,format_escale_name,'m_fragment_AreaHA_aux'), 'MapaBinario_ABCD1_pid','MapaBinario_ABCD1_pid_reclass','MapaBinario_ABCD1_pid_reclass_sttepings2', 'temp_BSSC','MapaBinario','A','MapaBinario_A','MapaBinario_AB','MapaBinario_ABC','MapaBinario_ABCD','MapaBinario_ABCD1','MapaBinario_ABCD1_pid','MapaBinario_ABCD1_pid_mode', '{}_FRAG{}{}'.format(i,meters,'m_mata_clump_pid_cross_corredor'),'MapaBinario_ABCD1_pid_reclass_sttepings']
        else:
  ...
  if prepare_biodim:
    ...
    for i in range(len(list_edge_depths)):
      ...
      create_TXTinputBIODIM(lista_maps_fid[:,i].tolist(), outputfolder = dirout, filename = "{}{}{}".format("simulados_HABMAT_FRAC_",mm,"m_PID"))
      create_TXTinputBIODIM(lista_maps_farea[:,i].tolist(), outputfolder = dirout, filename = "{}_{}{}".format(simulados_HABMAT_FRAC,mm,"m_AREApix"))
  ...
  if prepare_biodim:
    ...
    for i in range(len(list_gap_cross)):
      ...
      if mm != 0: # Do not export for gap crossing == 0
        create_TXTinputBIODIM(lista_maps_pid_clean[:,i].tolist(), outputfolder = dirout, filename = "simulados_HABMAT_grassclump_dila_{}{}".format(mm,"m_clean_PID"))
        create_TXTinputBIODIM(lista_maps_area_clean[:,i].tolist(), outputfolder = dirout, filename = "simulados_HABMAT_grassclump_dila_{}{}".format(mm,"m_clean_AREApix"))
        # If functional_area_complete == True, these statistics are also calculated
        if functional_area_complete:
          create_TXTinputBIODIM(lista_maps_pid_comp[:,i].tolist(), outputfolder = dirout, filename = "simulados_HABMAT_grassclump_dila_{}".format(mm,"m_complete_PID"))
          create_TXTinputBIODIM(lista_maps_area_comp[:,i].tolist(), outputfolder = dirout, filename = "simulados_HABMAT_grassclump_dila_{}{}".format(mm,"m_complete_AREApix"))
...
def edge_core(input_maps, list_edge_depths,
  ...
  for i_in in input_maps:
    ...
    for size in list_corridor_width_pixels:
      ...
      #espressaoEd=i+'_EDGE'+`format_escale_name`+'m_temp2 = int('+i+'_EDGE_'+`format_escale_name`+'m_temp1)' # criando uma mapa inteiro
      ...
      if export == True and dirout != '':
        ...
        grass.run_command('r.out.gdal', input = outputname_meco, output = outputname_meco+'.tif', overwrite = True)
        # Export edge map
        grass.run_command('r.out.gdal', input = outputname_edge, output = outputname_edge+'.tif', overwrite = True)
        # Export core map
        grass.run_command('r.out.gdal', input = outputname_core, output = outputname_core+'.tif', overwrite = True)

        # If the areas were calculated, export them too
        if calc_edge_core_area:
          # Export edge area clumps
          grass.run_command('r.out.gdal', input = edge_area_mapname, output = edge_area_mapname+'.tif', overwrite = True)
          # Export core area clumps
          grass.run_command('r.out.gdal', input = core_area_mapname, output = core_area_mapname+'.tif', overwrite = True)

          # If export_pid == True, export pid maps too
          if export_pid:
            # Export edge pid
            grass.run_command('r.out.gdal', input = edge_pid_mapname, output = edge_pid_mapname+'.tif', overwrite = True)
            # Export core pid
            grass.run_command('r.out.gdal', input = core_pid_mapname, output = core_pid_mapname+'.tif', overwrite = True)
...
def landscape_diversity(input_maps, scale_list, method = ['simpson'], alpha = [],
  ...
  for mm in input_maps:
    print("mapas de entrada: ", mm)
    #scalas
    for scale in scale_list:
      print("escala: ", scale)
      #metodos
      for met in method:
        if met == 'renyi' and (len(alpha) == 0 ):
          print("Executando  o metodo ", met, " na escala ", scale, "m para a mapa ", mm,' com alpha ', 0.5)
          renyi = RenyiIndex(mm, scale, alpha=0.5, export=export, export_path=dirout)
          renyi.compute()
        else:
          for a in alpha:
            print("Executando  o metodo ", met, " na escala ", scale, "m para a mapa ", mm,' com alpha ', a)
            raiseenyi = RenyiIndex(mm, scale, alpha=a, export=export, export_path=dirout)
            raiseenyi.compute()

        if met == "shannon":
          print("Executando  o metodo ", met, " na escala ", scale, "m para a mapa ", mm)
          shannon = ShannonFunction(mm, scale)
          shannon = ShannonFunction(mm,scale, export=export, export_path=dirout)
          shannon.compute()

        elif met == "simpson":
          print("Executando  o metodo ", met, " na escala ", scale, "m para a mapa ", mm)
          simpson = SimpsonFunction(mm,scale, export=export, export_path=dirout)
          simpson.compute()


        elif met == "pielou":
          print("Executando  o metodo ", met, " na escala ", scale, "m para a mapa ", mm)
          pielou = PielouFunction(mm,scale, export=export, export_path=dirout)
          pielou.compute()

    print('export +  dirout ',export, dirout)
    print('logica das diverdidades aqui ---------------------------------------')
...
class LSMetrics(wx.Panel):
    def __init__(self, parent, id):
        ...
        wx.StaticBitmap(self, -1, jpg0, (20, 15), (jpg0.GetWidth(), jpg0.GetHeight()), style=wx.SUNKEN_BORDER)

        # LEEC lab logo
        imageFile = 'logo_lab.png'
        im1 = Image.open(imageFile)
        jpg1 = wx.Image(imageFile, wx.BITMAP_TYPE_ANY).Scale(80, 82).ConvertToBitmap()
        wx.StaticBitmap(self, -1, jpg1, (230 + self.add_width, 15), (jpg1.GetWidth(), jpg1.GetHeight()), style=wx.SUNKEN_BORDER)
        ...
        self.Bind(wx.EVT_RADIOBOX, self.EvtRadioBox, id=91)
        ...
        self.Bind(wx.EVT_COMBOBOX, self.EvtComboBox, id=93)

        ############### do we need that here???
        #wx.EVT_TEXT(self, 93, self.EvtText)
        self.Bind(wx.EVT_TEXT, self.EvtText, id=93)
        ...
        self.Bind(wx.EVT_TEXT, self.EvtText, id=190)
        ...
        self.Bind(wx.EVT_CHECKBOX, self.EvtCheckBox, id=100)
        ...
        self.Bind( wx.EVT_CHECKBOX,  self.EvtCheckBox, id=51)
        ...
        self.Bind( wx.EVT_CHECKBOX,  self.EvtCheckBox, id=52)
        ...
        self.Bind( wx.EVT_TEXT,  self.EvtText, id=192)
        ...
        self.Bind( wx.EVT_CHECKBOX,  self.EvtCheckBox, id=53)
        ...
        self.Bind( wx.EVT_CHECKBOX,  self.EvtCheckBox, id=103)
        ...
        self.Bind( wx.EVT_CHECKBOX,  self.EvtCheckBox, id=54)
        ...
        self.Bind( wx.EVT_TEXT,  self.EvtText, id=193)
        ...
        self.Bind( wx.EVT_CHECKBOX,  self.EvtCheckBox, id=55)
        ...
        # Static text
        self.SelectMetrics9 = wx.StaticText(self, -1, "Metrics of functional connectivity:", wx.Point(520, 10))
        self.SelectMetrics9 = wx.StaticText(self, -1, "Export:", wx.Point(980, 10))

        #------------
        # Functionally connected area

        # Static text
        self.SelectMetrics10 = wx.StaticText(self, -1, "Functionally connected area:", wx.Point(520, 45))

        # Check box - event 105 (check calculate functionally connected area)
        self.insure12 = wx.CheckBox(self, 105, '', wx.Point(690, 45))
        #wx.EVT_CHECKBOX(self, 105, self.EvtCheckBox)
        self.Bind( wx.EVT_CHECKBOX,  self.EvtCheckBox, id=105)

        # Static text
        self.SelectMetrics11 = wx.StaticText(self, -1, "Crossing distance (m):", wx.Point(720, 45))

        # Text Control - event 194
        # List of gap crossing distances for calculating functional connectivity
        self.editname5 = wx.TextCtrl(self, 194, '', wx.Point(850, 40), wx.Size(120,-1))
        #wx.EVT_TEXT(self, 194, self.EvtText)
        self.Bind( wx.EVT_TEXT,  self.EvtText, id=194)
        self.editname5.Disable()

        # Check Box - event 56 (export maps of functionally connected area)
        self.insure13 = wx.CheckBox(self, 56, "", wx.Point(990, 47))
        #wx.EVT_CHECKBOX(self, 56, self.EvtCheckBox)
        self.Bind( wx.EVT_CHECKBOX,  self.EvtCheckBox, id=56)
        ...
        self.insure14 = wx.CheckBox(self, 106, 'Functional connectivity', wx.Point(520,80))
        #wx.EVT_CHECKBOX(self, 106, self.EvtCheckBox)
        self.Bind( wx.EVT_CHECKBOX,  self.EvtCheckBox, id=106)
        ...
        self.insure15 = wx.CheckBox(self, 107, 'Complete funct. connected area', wx.Point(690, 80))
        #wx.EVT_CHECKBOX(self, 107, self.EvtCheckBox)
        self.Bind( wx.EVT_CHECKBOX,  self.EvtCheckBox, id=107)
        ...
        # Static text
        self.SelectMetrics12 = wx.StaticText(self, -1, "Metrics of edge:", wx.Point(520, 114))

        #------------
        # Maps of distance from edges

        # Check box - event 108 (check calculate distance from edges)
        self.insure16 = wx.CheckBox(self, 108, 'Map of distance from edges', wx.Point(690,114))
        #wx.EVT_CHECKBOX(self, 108, self.EvtCheckBox)
        self.Bind( wx.EVT_CHECKBOX,  self.EvtCheckBox, id=108)

        # Check Box - event 57 (export maps of distance from edges)
        self.insure16_5 = wx.CheckBox(self, 57, "", wx.Point(990, 114))
        #wx.EVT_CHECKBOX(self, 57, self.EvtCheckBox)
        self.Bind( wx.EVT_CHECKBOX,  self.EvtCheckBox, id=57)
        ...
        # Static text
        self.SelectMetrics13 = wx.StaticText(self, -1, "Classify edge/core/matrix:", wx.Point(520, 145))

        # Check box - event 109 (check classify edge/core/matrix)
        self.insure17 = wx.CheckBox(self, 109, '', wx.Point(690, 145))
        #wx.EVT_CHECKBOX(self, 109, self.EvtCheckBox)
        self.Bind( wx.EVT_CHECKBOX,  self.EvtCheckBox, id=109)

        # Static text
        self.SelectMetrics14 = wx.StaticText(self, -1, "Edge depths (m):", wx.Point(710, 145))

        # Text Control - event 195
        # List of edge depths for calculating edge/core maps
        self.editname6 = wx.TextCtrl(self, 195, '', wx.Point(850, 140), wx.Size(120,-1))
        #wx.EVT_TEXT(self, 195, self.EvtText)
        self.Bind( wx.EVT_TEXT,  self.EvtText, id=195)

        self.editname6.Disable()

        # Check Box - event 58 (export maps of edge/core/matrix)
        self.insure18 = wx.CheckBox(self, 58, "", wx.Point(990, 147))
        #wx.EVT_CHECKBOX(self, 58, self.EvtCheckBox)
        self.Bind( wx.EVT_CHECKBOX,  self.EvtCheckBox, id=58)
        ...
        # Static text
        self.SelectMetrics15 = wx.StaticText(self, -1, "Proportion of edge/core:", wx.Point(520, 175))

        # Check box - event 110 (check calculate proportion of edge/core)
        self.insure19 = wx.CheckBox(self, 110, '', wx.Point(690, 175))
        #wx.EVT_CHECKBOX(self, 110, self.EvtCheckBox)
        self.Bind( wx.EVT_CHECKBOX,  self.EvtCheckBox, id=110)
        self.insure19.Disable()

        # Static text
        self.SelectMetrics16 = wx.StaticText(self, -1, "Window size (m):", wx.Point(710, 175))

        # Text Control - event 196
        # List of moving window sizes for calculating proportion of edge/core
        self.editname7 = wx.TextCtrl(self, 196, '', wx.Point(850, 170), wx.Size(120,-1))
        #wx.EVT_TEXT(self, 196, self.EvtText)
        self.Bind(  wx.EVT_TEXT,  self.EvtText, id=196)
        ...
        self.insure20 = wx.CheckBox(self, 111, 'Calculate area of edge/core clumps?', wx.Point(520, 205))
        #wx.EVT_CHECKBOX(self, 111, self.EvtCheckBox)
        self.Bind( wx.EVT_CHECKBOX,  self.EvtCheckBox, id=111)
        ...
        # Static text
        self.SelectMetrics16 = wx.StaticText(self, -1, "Landscape diversity:", wx.Point(520, 228))

        # Check box - event 112 (check calculate diversity metrics)
        self.insure21 = wx.CheckBox(self, 112, '', wx.Point(690, 228))

        #wx.EVT_CHECKBOX(self, 112, self.EvtCheckBox)
        self.Bind( wx.EVT_CHECKBOX,  self.EvtCheckBox, id=112)

        # Static text
        self.SelectMetrics17 = wx.StaticText(self, -1, "Window size (m):", wx.Point(710, 228))

        # Text Control - event 197
        # List of window sizes for calculating diversity maps
        self.editname8 = wx.TextCtrl(self, 197, '', wx.Point(850, 220), wx.Size(120,-1))
        #wx.EVT_TEXT(self, 197, self.EvtText)
        self.Bind( wx.EVT_TEXT,  self.EvtText, id=197)
        self.editname8.Disable()

        # Check Box - event 59 (export maps of diversity)
        self.insure22 = wx.CheckBox(self, 59, "", wx.Point(990, 228))
        #wx.EVT_CHECKBOX(self, 59, self.EvtCheckBox)
        self.Bind( wx.EVT_CHECKBOX,  self.EvtCheckBox, id=59)
        self.insure22.Disable()

        # Static text
        self.SelectMetrics18 = wx.StaticText(self, -1, "Index:", wx.Point(520, 259))

        # Check box - event 130 (check method for diversity metrics - Shannon)
        self.insure23 = wx.CheckBox(self, 130, 'Shannon', wx.Point(560, 259))
        #wx.EVT_CHECKBOX(self, 130, self.EvtCheckBox)
        self.Bind( wx.EVT_CHECKBOX,  self.EvtCheckBox, id=130)
        self.insure23.Disable()

        # Check box - event 131 (check method for diversity metrics - Simpson)
        self.insure24 = wx.CheckBox(self, 131, 'Simpson', wx.Point(630, 259))
        #wx.EVT_CHECKBOX(self, 131, self.EvtCheckBox)
        self.Bind( wx.EVT_CHECKBOX,  self.EvtCheckBox, id=131)
        self.insure24.Disable()

        # Check box - event 132 (check method for diversity metrics - Pielou)
        self.insure25 = wx.CheckBox(self, 132, 'Pielou', wx.Point(700, 259))
        #wx.EVT_CHECKBOX(self, 132, self.EvtCheckBox)
        self.Bind( wx.EVT_CHECKBOX,  self.EvtCheckBox, id=132)
        self.insure25.Disable()

        # Check box - event 133 (check method for diversity metrics - Renyi)
        self.insure26 = wx.CheckBox(self, 133, 'Renyi', wx.Point(760, 259))
        #wx.EVT_CHECKBOX(self, 133, self.EvtCheckBox)
        self.Bind( wx.EVT_CHECKBOX,  self.EvtCheckBox, id=133)
        self.insure26.Disable()

        # Static text
        self.SelectMetrics19 = wx.StaticText(self, -1, "Alpha:", wx.Point(820, 259))

        # Text Control - event 198
        # List of alpha values for landscape diversity index Renyi
        self.editname9 = wx.TextCtrl(self, 198, '0.5', wx.Point(870, 248), wx.Size(60, -1))
        #wx.EVT_TEXT(self, 198, self.EvtText)
        self.Bind( wx.EVT_TEXT,  self.EvtText, id=198)
        ...
        self.button = wx.Button(self, 10, "START CALCULATIONS", wx.Point(520, 290))
        #wx.EVT_BUTTON(self, 10, self.OnClick)
        self.Bind( wx.EVT_BUTTON,  self.OnClick, id=10)

        # Button - event 8 - Exit LSMetrics
        self.button = wx.Button(self, 8, "EXIT", wx.Point(700, 290))
    ...
    def OnClick(self,event):
        ...
        if event.GetId() == 8:
            # Cria um dialog modal com a mensagem
            d = wx.MessageDialog(self, "Exiting the program!\n"
                                 "", "Thanks!", wx.OK)        # mostra a janela
            d.ShowModal()   # espera a resposta do usuário
            wx.MilliSleep(3000)  # espera 3000 ms = 3 segundos
                 # destrói o dialog
            self.Close()         # fecha o frame principal
            wx.GetApp().ExitMainLoop()  # garante que toda a app feche
